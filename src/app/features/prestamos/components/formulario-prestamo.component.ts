    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm" (click)="closeModal()">
      <div class="bg-white dark:bg-slate-800 rounded-xl shadow-xl w-full max-w-lg overflow-hidden" (click)="$event.stopPropagation()">
        <div class="px-6 py-4 border-b border-slate-100 dark:border-slate-700 flex justify-between items-center bg-slate-50 dark:bg-slate-900">
          <h2 class="text-lg font-bold text-slate-800 dark:text-white">Registrar Nuevo Préstamo</h2>
        ...
        <form [formGroup]="prestamoForm" (ngSubmit)="onSubmit()" class="p-6 space-y-4">
          <div>
            <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Usuario Solicitante</label>
            <input type="text" formControlName="usuario" class="w-full rounded-lg border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 text-slate-900 dark:text-white focus:ring-primary focus:border-primary placeholder-slate-400" placeholder="Ej: pepito.juares">
          </div>

          <div>
             <div class="flex justify-between items-center mb-2">
               <label class="block text-sm font-medium text-slate-700 dark:text-slate-300">Ítems</label>
               <button type="button" (click)="addItem()" class="text-xs text-primary dark:text-blue-400 font-medium hover:underline">+ Agregar Ítem</button>
             </div>
             
             <div formArrayName="items" class="space-y-3 max-h-60 overflow-y-auto pr-2">
               <div *ngFor="let item of items.controls; let i=index" [formGroupName]="i" class="flex gap-2 items-start p-2 bg-slate-50 dark:bg-slate-900/50 rounded-lg border border-slate-200 dark:border-slate-700">
                 <div class="flex-1 space-y-2">
                   <select formControlName="categoria" class="w-full text-xs rounded border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-800 text-slate-900 dark:text-white">
                     <option value="PERIFERICO">Periférico</option>
                     <option value="EQUIPO">Equipo (Activo)</option>
                     <option value="OTRO">Otro</option>
                   </select>
                   <input type="text" formControlName="nombre" placeholder="Nombre (Ej: Mouse)" class="w-full text-sm rounded border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-800 text-slate-900 dark:text-white placeholder-slate-400">
                   <input *ngIf="item.get('categoria')?.value === 'EQUIPO'" type="text" formControlName="serial" placeholder="Serial (Ej: PC-123)" class="w-full text-xs rounded border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-800 text-slate-900 dark:text-white font-mono placeholder-slate-400">
                 </div>
                 <button type="button" (click)="removeItem(i)" class="p-1 text-red-400 hover:text-red-600 dark:hover:text-red-300">
