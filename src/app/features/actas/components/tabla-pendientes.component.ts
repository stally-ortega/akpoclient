      ...
      <table class="min-w-full divide-y divide-slate-200">
        ...
        <tbody class="bg-white divide-y divide-slate-200">
          <tr *ngFor="let acta of actas" class="hover:bg-slate-50">
            ...
            <td class="px-6 py-4 text-sm text-slate-600">{{ acta.usuarioSolicitante }}</td>
            <td class="px-6 py-4">
          ...
          <tr *ngIf="actas.length === 0">
            <td colspan="6" class="px-6 py-8 text-center text-slate-500">No hay actas pendientes</td>
          </tr>
        </tbody>
      </table>
    </div>
